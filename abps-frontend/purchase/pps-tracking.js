...
async function savePPSDeliverySchedule(prnId, btn) {
  ...
  try {
    ...
    if (data.success) {
      ...
      // PPS Document (11 Sep 2026, rebuilt PER-PRN 16 Sep 2026) — one
      // document per PRN now (docUrls is keyed by prnId), so this screen
      // (which only ever saves one PRN at a time) shows at most one link.
      // A save that regenerated no document (e.g. the regeneration itself
      // failed server-side, logged but non-fatal) simply shows no link
      // rather than an error — the schedule save itself already succeeded
